...
// Memoized PlatformCard component
const PlatformCard = memo(({ platform, variants }) => (
  <motion.div variants={variants}>
    <div className="bg-muted/60 dark:bg-muted/70 backdrop-blur-md border border-border/40 dark:border-border/60 rounded-2xl shadow-xl p-6 flex flex-col items-center text-center h-full transition-all duration-300 hover:shadow-2xl hover:border-primary/40 hover:-translate-y-1 transform-gpu">
...
export default function CP() {
  ...
  const highlightsMotionProps = useMemo(() => ({
      ...
      >
        ...
        >
          {cpPlatforms.map((platform, index) => (
            <PlatformCard key={index} platform={platform} variants={cardVariants} />
          ))}
        </motion.div>

        <motion.div
          {...highlightsMotionProps}
          className="relative z-10"
        >
          <div className="bg-muted/60 dark:bg-muted/70 backdrop-blur-md border border-border/40 dark:border-border/60 rounded-2xl shadow-xl p-6 transition-all duration-300 hover:shadow-2xl hover:border-primary/40 hover:-translate-y-1 transform-gpu">
